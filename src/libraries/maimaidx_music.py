    ...
    def filter(self,
        ...
        for music in self:
            ...
            if artist is not Ellipsis and artist.lower() not in music.artist.lower():
                continue
            if version is not Ellipsis and version.lower() not in music.version.lower():
                continue
            # 紫谱
            if len(music.charts) == 4:
                if charter is not Ellipsis and charter.lower() not in music.charts[2].charter.lower() \
                        and charter.lower() not in music.charts[3].charter.lower():
                    # print(music.charts.charter)
                    continue
            else:
                if charter is not Ellipsis and charter.lower() not in music.charts[2].charter.lower() \
                        and charter.lower() not in music.charts[3].charter.lower() \
                        and charter.lower() not in music.charts[4].charter.lower():
                    # print(music.charts.charter)
                    continue
            music.diff = diff2
