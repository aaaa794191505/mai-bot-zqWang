...
# id, difficult难度, type类型, acc完成度, rating分数, combo连击, score分数, title, ds定数 ,lv
class ChartInfo(object):
    ...
    def __str__(self):
        return '%-50s' % f'{self.title} [{self.tp}]' + f'{self.ds}\t{diffs[self.diff]}\t{self.ra}\t{self.lv}'
...
class BestList(object):
    ...
    def sort_by_acc(self):
        self.data.sort(key=lambda chartinfo: chartinfo.achievement, reverse=True)
...
class DrawBest(object):
    ...
    def _drawBestList(self, img: Image.Image, sdBest: BestList, dxBest: BestList):
        ...
        for num in range(0, len(sdBest)):
            ...
            # 修改pngPath = self.cover_dir + f'{chartInfo.idNum}.jpg'
        ...
        for num in range(0, len(dxBest)):
            ...
            # 修改pngPath = self.cover_dir + f'{int(chartInfo.idNum)}.jpg'
            # if not os.path.exists(pngPath):
...
async def generate(payload: Dict) -> (Optional[Image.Image], BestList, bool):
    async with aiohttp.request("POST", "https://www.diving-fish.com/api/maimaidxprober/query/player",
                               json=payload) as resp:
        # async with aiohttp.request("POST", "https://www.diving-fish.com/api/maimaidxprober/player/records",
        #                            json=payload) as resp:
        if resp.status == 400:
        ...
        for c in sd:
            ...
            t_list.push(ChartInfo.from_json(c))
        for c in dx:
            dx_best.push(ChartInfo.from_json(c))
            t_list.push(ChartInfo.from_json(c))
        # print(t_list.data[0].__str__())

        pic = DrawBest(sd_best, dx_best, obj["nickname"], obj["rating"] + obj["additional_rating"],
                       obj["rating"]).getDir()
        if "username" in str(payload):
            # print("含账号")
            return pic, t_list, 423
        else:
            return pic, t_list, 0


async def generate_all(payload: Dict) -> (Optional[Image.Image], List, bool):
    async with aiohttp.request("POST", "https://www.diving-fish.com/api/maimaidxprober/query/plate",
        ...
        print("共有" + str(len(obj['verlist'])) + "条数据")
        # return None, None, 500
        # "verlist": [{"achievements": 99.4629, "fc": "", "fs": "", "id": 71, "level": "11",
        # "level_index": 3,
        #              "title": "\u30de\u30c8\u30ea\u30e7\u30b7\u30ab", "type": "SD"}
        for c in obj['verlist']:
            music = total_list.by_id(str(c['id']))
            temp = c
            # print(music)
            if music is not None:
                temp['ds'] = music['ds'][temp['level_index']]
            t_list.append(temp)
        # print(t_list[2])
        ...
        if "username" in str(payload):
            # print("含账号")
            return None, t_list, 423
        return None, t_list, 0
